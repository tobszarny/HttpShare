...
public class HTTPSession implements IHTTPSession {
    ...
    private void decodeHeader(BufferedReader in, Map<String, String> pre, Map<String, String> parms, Map<String, String> headers) throws ResponseException {
        try {
            ...
            if (!st.hasMoreTokens()) {
                throw new ResponseException(ResponseStatus.BAD_REQUEST, "BAD REQUEST: Syntax error. Usage: GET /example/file.html");
            }

            pre.put("method", st.nextToken());

            if (!st.hasMoreTokens()) {
                throw new ResponseException(ResponseStatus.BAD_REQUEST, "BAD REQUEST: Missing URI. Usage: GET /example/file.html");
            }
            ...
            throw new ResponseException(ResponseStatus.INTERNAL_ERROR, "SERVER INTERNAL ERROR: IOException: " + ioe.getMessage(), ioe);
        }
    }
    ...
    private void decodeMultipartFormData(ContentType contentType, ByteBuffer fbuf, Map<String, String> parms, Map<String, String> files) throws ResponseException {
        ...
        try {
            ...
            if (boundaryIdxs.length < 2) {
                throw new ResponseException(ResponseStatus.BAD_REQUEST, "BAD REQUEST: Content type is multipart/form-data but contains less than two boundary strings.");
            }
            ...
            for (int boundaryIdx = 0; boundaryIdx < boundaryIdxs.length - 1; boundaryIdx++) {
                ...
                if (mpline == null || !mpline.contains(contentType.getBoundary())) {
                    throw new ResponseException(ResponseStatus.BAD_REQUEST, "BAD REQUEST: Content type is multipart/form-data but chunk does not start with boundary.");
                }
                ...
                if (partHeaderLength >= len - 4) {
                    throw new ResponseException(ResponseStatus.INTERNAL_ERROR, "Multipart header size exceeds MAX_HEADER_SIZE.");
                }
            ...
            throw new ResponseException(ResponseStatus.INTERNAL_ERROR, e.toString());
        }
    }
    ...
    public void execute() throws IOException {
        ...
        try {
            ...
            try {
                ...
                NetworkUtil.safeClose(this.outputStream);
            ...
            if (this.method == null) {
                throw new ResponseException(ResponseStatus.BAD_REQUEST, "BAD REQUEST: Syntax error. HTTP verb " + pre.get("method") + " unhandled.");
            }
            ...
            if (r == null) {
                throw new ResponseException(ResponseStatus.INTERNAL_ERROR, "SERVER INTERNAL ERROR: Serve() returned a null response.");
            } else {
            ...
            Response resp = NanoHTTPD.newFixedLengthResponse(ResponseStatus.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "SSL PROTOCOL FAILURE: " + ssle.getMessage());
            resp.send(this.outputStream);
            NetworkUtil.safeClose(this.outputStream);
        } catch (IOException ioe) {
            Response resp = NanoHTTPD.newFixedLengthResponse(ResponseStatus.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "SERVER INTERNAL ERROR: IOException: " + ioe.getMessage());
            resp.send(this.outputStream);
            NetworkUtil.safeClose(this.outputStream);
        } catch (ResponseException re) {
        ...
        try {
            ...
            if (Method.POST.equals(this.method)) {
                ...
                if (contentType.isMultipart()) {
                    ...
                    if (boundary == null) {
                        throw new ResponseException(ResponseStatus.BAD_REQUEST,
            ...
            NetworkUtil.safeClose(randomAccessFile);
        }
    }
    ...
    private String saveTmpFile(ByteBuffer b, int offset, int len, String filename_hint) {
        ...
        if (len > 0) {
            ...
            try {
                ...
                NetworkUtil.safeClose(fileOutputStream);
            }
        }
        return path;
    }
